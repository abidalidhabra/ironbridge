...
<div class="right_paddingboxpart">
    <div class="users_datatablebox">
        <div class="">               
            ...
            <div class="col-md-12">
                <div class="row">
                    <h3>Edit Avatar | Agent Levels</h3>
                </div>
            </div>
        </div>
    </div>
                        ...
                        <div class="form-group @error('agent_level') has-error @enderror">
                            ...
                            <select name="agent_level" class="form-control">
                                ...
                                @forelse($agent_complementary as $agent)
                                    <option value="{{ $agent->agent_level }}" @if($agent->agent_level==$relicReward->agent_level){{ 'selected' }}@endif >{{ $agent->agent_level }}</option>
                                @empty
                                @endforelse
                            </select>
                        </div>
...
<script>
    ...
    $(document).on('submit', '#editRelicRewardForm', function(e) {
        e.preventDefault();
        let url = "{{ route('admin.avatar-agent-levels.update',$relicReward->id) }}";
        // if(validate()) {
            $.ajax({
                type: "POST",
                url: url,
